...
// 兼容validator表单验证。
    if($.validator){
            ...
            showErrors: function(errorMap, errorList){
                ...
                if(!!errorList.length){
                    ...
                    for(var i = 0, len = errorList.length; i < len; i++){
                        var placement = 'auto top-right',
                            element = errorList[i].element,
                            $element = $(element),
                            $elementFormGroup = $element.parents('.form-group:eq(0)');
                        
                        if($elementFormGroup.offset().top < 54){
                            placement = 'auto bottom-right';
                        }
                        $elementFormGroup.tooltip('destroy');
                        $elementFormGroup.tooltip({
                            container:'body',
                            placement: placement,
                            animation: false,
                        ...
                        // aidicon aidicon aidicon-alert-circle
                        $elementFormGroup.addClass('has-error has-icon');
                        if( $elementFormGroup.find('i.aidicon.aidicon-alert-circle') &&  $elementFormGroup.find('i.aidicon.aidicon-alert-circle').length > 0){
                            $elementFormGroup.find('i.aidicon.aidicon-alert-circle').removeClass("validate-hidden");
                        }else{
                            $elementFormGroup.append("<i class='aidicon aidicon-alert-circle'></i>");     
                        }
                    }

                }else{
                    var len = this.successList.length;

                    for(var i = 0; i < len; i++){
                        var element = this.successList[i],
                            $element = $(element),
                            $elementFormGroup = $element.parents('.form-group:eq(0)');
                        $elementFormGroup.find('i.aidicon.aidicon-alert-circle').addClass('validate-hidden');
                        $elementFormGroup.removeClass('has-error has-icon');
                        $elementFormGroup.tooltip('destroy');
                    }
                }
            }
        ...
        $.validator.addMethod("cnDateTime", function(value, element) { 
            var ereg = /^\d{4}年\d{2}月\d{2}日$/;
			return ereg.test(value) && moment(value,'YYYY年MM月DD').isValid();
        }, "请输入正确格式的日期(YYYY-MM-DD HH:mm:ss)！");

        $.validator.addMethod("chineseDate", function(value, element) { 
            var ereg = /^\d{4}年\d{2}月\d{2}日$/;
			return ereg.test(value) && moment(value,'YYYY年MM月DD').isValid();
        }, "请输入正确格式的日期(yyyy年MM月dd)！");
        ...
        $.validator.addMethod("specialStr", function(value, element) { 
		...
		// 自定义验证输入的非法字符
		// invalidChar:"'`~^\""
        $.validator.addMethod("invalidChar", function(value, element, param) { 
