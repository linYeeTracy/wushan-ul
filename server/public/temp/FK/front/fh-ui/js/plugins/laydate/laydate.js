...
;!function(){
  ...
  */
  ...
  Class.prototype.config = {
    ...
    // 新增
    ,shortcuts:[]
    ,showWeek: false  // 周显示的情况下 必须第一天为周一
    ,mondayStart: false //是否以周一为一周开始，默认以周日开始 
  };
  ...
  Class.prototype.lang = function(){
    ...
    ,text = {
      ...
      ,en: {
        weeks: ['Week','Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa']
  ...
  Class.prototype.init = function(){
    ...
    if(options.showWeek === true)  that.config.mondayStart = true;
  ...
  Class.prototype.render = function(){
    ...
    lay.each(new Array(2), function(i){
      ...
      lay.each(new Array(6), function(i){ //表体
        ...
        if(options.showWeek === false){
            lay.each(new Array(7), function(j){
                if(i === 0){
                    var th = lay.elem('th');
                    // th.innerHTML = lang.weeks[j+1];
                    th.innerHTML = options.mondayStart ? lang.weeks[j+2] : lang.weeks[j+1];
                    if(options.mondayStart && j === 6){
                        th.innerHTML = lang.weeks[1];
                    }
            ...
            lay.each(new Array(8), function(j){
                if(i === 0){
                    var th = lay.elem('th');
                    if(j===0){
                        th.innerHTML = lang.weeks[0];
                    }else if(j===7){
                        th.innerHTML = lang.weeks[1];
                    }else{
                        th.innerHTML = lang.weeks[j+1];
                    }
                    j===0 && lay(th).addClass('weekTh');
                    theadTr.appendChild(th);
                    tr.insertCell(j);
                }else{
                    tr.insertCell(j);
    ...
    lay(divFooter).html(function(){
       ...
       * 2018-6-1 快捷选择
       */
        if (that.config.shortcuts.length > 0) {
            for(var j = 0; j < that.config.shortcuts.length; j++ ){
                shortcuts.push(that.config.shortcuts[j]);
            }
        }
        /*
        * 2018-6-1 快捷选择
  ...
  Class.prototype.calendar = function(value){
    ...
    if(options.showWeek === false && options.range === false){
        //赋值日
        ...
        //赋值日
        lay.each(tds, function(index, item){
        if(index%8 === 0 && options.showWeek === true){// 增加周
             var YMD = [dateTime.year, dateTime.month], st = 0,
            item = lay(item);
            item.removeAttr('class');
            index = index - resetIndex + 1;
            if(index < startWeek){//上一月
                st = prevMaxDate - startWeek + index;
                YMD = that.getAsYM(dateTime.year, dateTime.month, 'sub');
            } else if(index >= startWeek && index < thisMaxDate + startWeek){//本月
                st = index - startWeek;
            } else {//下一月
                st = index - thisMaxDate - startWeek;
                YMD = that.getAsYM(dateTime.year, dateTime.month);
            ...
            var week = laydate.getYearWeek(new Date(YMD[0],YMD[1]-1,YMD[2]))
            item.attr('lay-week', week).html(week);
            resetIndex++;
        }else{
            var YMD = [dateTime.year, dateTime.month], st = 0;
            item = lay(item);
            item.removeAttr('class');
            index = index - resetIndex;
            if(index < startWeek){
                st = prevMaxDate - startWeek + index;
                item.addClass('laydate-day-prev');
                YMD = that.getAsYM(dateTime.year, dateTime.month, 'sub');
    ...
    if(type === 'year' || type === 'month'){      
      ...
      lay(ul).find('li').on('click', function(){
        ...
        if(index === 0){
            ...
            // 如果是年或者月模式，选中日期就关闭弹框
            if ((type == "month" && that.config.type == "month" && that.config.range === false) || (type == "year" && that.config.type == "year" && that.config.range === false)) {
      ...
      var span = lay.elem('span', {
        ...
        lay(ul).find('ol').each(function(i){
          var ol = this
          ,li = lay(ol).find('li')
          ol.scrollTop = 36*(that[startEnd][hms[i]]);
          if(ol.scrollTop <= 0){
  ...
  Class.prototype.stampRange = function(){
    ...
    lay.each(tds, function(i, item){
        if(lay(item).attr('lay-ymd') !== null && lay(item).attr('lay-ymd') !== undefined){
  ...
  Class.prototype.changeEvent = function(){
    ...
    lay.each(that.table, function(i, table){
      var tds = lay(table).find('td:not([lay-week])');
  ...
  laydate.getEndDate = function(month, year){
   ...
   //获取周数
  laydate.getYearWeek = function(date){
